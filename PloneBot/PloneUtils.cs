...
namespace PloneBot
{
	static class PloneUtils
	{
		...
		public enum Jobs { Unknown, SetupPortletsFolder, SavePortletChanges };

		public struct Message
		{
			public Jobs job;
			public string user, title, description;
			...
			public Message(string message)
			{
				...
				{
					case "setup portlets folder":
						job = Jobs.SetupPortletsFolder;
						break;
					case "save portlet changes":
						job = Jobs.SavePortletChanges;
						break;
					default:
						job = Jobs.Unknown;
						break;
				}
				creatorId = Convert.ToUInt32(messageReader.ReadLine());
				user = messageReader.ReadLine();
				uri = new Uri(messageReader.ReadLine() + '/');
				title = messageReader.ReadLine();
				description = messageReader.ReadToEnd();
			}
		}
